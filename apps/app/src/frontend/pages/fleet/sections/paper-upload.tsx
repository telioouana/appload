...
import { requiresExpiry, subjectKind } from "@workspace/domain/kyc/requirements"
 ...
 * A mutation that fails leaves blobs that expire on their own rather than an
 * orphaned ID scan sitting in storage.
 */
...
/**
 * One page of a filed paper, against the proxied href — the storage URL is
 * never sent to the browser. PDFs render in an `<object>` and images inline;
 * anything else falls back to a link, so a page that cannot be drawn is
 * still reachable.
 */
export function PaperPreview({ page, index }: { page: KycPage; index: number }) {
    const t = useTranslations("App.fleet.papers")
