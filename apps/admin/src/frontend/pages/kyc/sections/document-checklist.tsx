...
import type { KycDocumentType, KycSubjectType } from "@workspace/db/types"
...
/** The signed-in staff member's role, defaulting to the least privileged one. */
export function useStaffRole(): StaffRole {
    ...
    return isStaffRole(session?.user.role) ? session.user.role : "user"
}

/**
 * The review surface: the subject's required-document checklist, the pages
 * of whichever document is open, and the decision form. Lives inside the
 * profile panel's Documents tab.
 */
export function DocumentChecklist({ subjectType, subjectId }: { subjectType: KycSubjectType; subjectId: string }) {
    ...
    const invalidate = async () => {
        ...
        // list has to refetch alongside the checklist
        await Promise.all([
                ...
                <>
                    ...
                    <div className="flex flex-col gap-3">
                        {REQUIRED_DOCS[kind].map((requirement) => {
                            // A slot may accept more than one document type (a
                            // driver proves identity with either a licence or an
                            // ID card). Every type actually present is rendered —
                            // showing only the first would hide a second
                            // submission from review entirely.
                            ...
                            return slotTypes.map((type) => {
                                ...
                                return (
                                    <div key={type} className="flex flex-col gap-3">
                                        ...
                                        {/* Upload is offered where the slot is empty,
                                            and as a resubmission when the document
                                            was turned down */}
