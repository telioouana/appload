...
export const invoicePartyOf = (type: OrderDocumentType): DocumentParty | null =>
...
export const formType = (type: OrderDocumentType): FormType => (invoicePartyOf(type) ? INVOICE_OPTION : (type as FormType))
...
/**
 * What the dialog opens on: "Record payment" passes a proof for that leg,
 * the Add button passes nothing. No nonce any more — the dialog is mounted
 * per open, so every open already starts clean.
 */
...
/**
 * Adding a document to an order: a POD or an invoice, a debit or credit note
 * that shifts the order's effective totals, a proof of payment that drives
 * the leg's paid columns, or evidence for a status change.
 *
 * Mount it per open (`{adding && <AddDocumentDialog … />}`): the draft, the
 * seeded invoice fields and the payment summary then all start fresh, which
 * is what lets the preset be a plain prop rather than something the form has
 * to de-duplicate against itself.
 */
export function AddDocumentDialog({
    ...
    const [draft, setDraft] = useState<Draft>(() => (initial
        ? { ...emptyDraft(), type: formType(initial.type), party: initial.party ?? invoicePartyOf(initial.type) ?? undefined }
        : emptyDraft()))
    ...
    const summary = useQuery(
        trpc.documents.paymentSummary.queryOptions({ orderId }, { enabled: isPop }),
    )
                ...
                <div className="flex flex-col gap-3 rounded-xl border p-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className="ml-auto flex gap-2">
                            <Button type="button" size="sm" variant="ghost" disabled={submitting} onClick={onClose}>
                                <IconX />
                                {t("form.cancel")}
                            </Button>
