...
import type { StaffThreadRow } from "@workspace/domain/threads/queries";
...
/** The two conversations an order has: with its driver, and with its parties. */
export const CHAT_MODES = ["drivers", "orders"] as const;
    ...
    return (
        <aside className="flex w-80 shrink-0 flex-col border-r">
            <div className="flex items-start justify-between gap-2 p-4 pb-3">
                ...
                {/* Only a driver conversation is started from here: an order's
                    own thread opens the first time somebody writes on it */}
                {!onOrders && (
            ...
            {isLoading ? (
                ...
                <Empty className="py-10">
                    <EmptyHeader>
                        ...
                        <EmptyTitle>{t("threads.listEmptyTitle")}</EmptyTitle>
                        <EmptyDescription>{t("threads.listEmptyDescription")}</EmptyDescription>
                    </EmptyHeader>
                </Empty>
            ) : (
                <ItemGroup className="gap-1 p-2">
                                    ...
                                    <ItemContent className="min-w-0">
                                        <ItemTitle className="flex w-full items-center justify-between gap-2">
                                            ...
                                            {thread.lastMessageAt && (
                                                <span className="shrink-0 text-xs font-normal text-muted-foreground">
                                                    {f.relativeTime(thread.lastMessageAt, now)}
                                                </span>
                                            )}
                                        </ItemTitle>
                                        <ItemDescription className="flex w-full items-center justify-between gap-2">
                                            <span className={cn("min-w-0 truncate", showUnread && "font-medium text-foreground")}>
                                                {/* Every listed thread has been written in; a
                                                    blank preview is a message that carried only
                                                    files */}
                                                {thread.lastMessage || t("threads.fileOnly")}
                                            </span>
                                            {showUnread && (
                                                <Badge
                                                    className="shrink-0 px-1.5"
                                                    aria-label={t("list.unreadCount", { count: thread.unread })}
                                                >
                                                    {thread.unread}
                                                </Badge>
                                            )}
                                        ...
                                        <ItemDescription className="truncate text-xs">
                                            {[thread.shipperName, thread.carrierName ?? t("threads.noCarrier")]
                                                .filter(Boolean)
                                                .join(" · ")}
                                        </ItemDescription>
                                    </ItemContent>
