 ...
 * dimmed rather than removed — an operator looking for one truck still sees
...
export function matchesOrder(order: MapOrder, query: string): boolean {
    const term = normalizeQuery(query)

    if (!term) return true

    const fields = [order.orderId, order.shipperName, order.carrierName, order.driverName]

    if (fields.some((field) => field && normalizeQuery(field).includes(term))) return true

    const plate = plateKey(term)

    return plate.length > 0 && !!order.truckPlate && plateKey(order.truckPlate).includes(plate)
}
